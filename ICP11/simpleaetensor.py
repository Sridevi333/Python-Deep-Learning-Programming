...
with tf.Session() as sess:
    sess.run(init)
    summary_writer = tf.summary.FileWriter('./graphs/simsc', graph=tf.get_default_graph())
    print('Training...')
    for i in range(1001):
        batch_xs, batch_ys = mnist.train.next_batch(128)
        train_step.run({x: batch_xs, y_: batch_ys})
        _, l, summary = sess.run([train_step, loss, summa], feed_dict={x: batch_xs, y_: batch_ys})
        # Write logs at every iteration
        summary_writer.add_summary(summary, 10001 * 128 + i)
